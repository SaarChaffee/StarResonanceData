...
local mergeDataFuncs = {
  ...
  [6] = function(container, buffer, watcherList)
    ...
    local t = {}
    local last = container.__data__.preBuyUser
    container.__data__.preBuyUser = t
    for i = 1, count do
      local v = br.ReadInt64(buffer)
      t[#t + 1] = v
      container.Watcher:MarkDirty("preBuyUser", last)
    end
  end
}
...
local resetData = function(container, pbData)
  ...
  if not pbData.preBuyUser then
    container.__data__.preBuyUser = {}
  end
...
local getContainerElem = function(container)
  ...
  if container.preBuyUser ~= nil then
    local data = {}
    for index, repeatedItem in pairs(container.preBuyUser) do
      data[index] = {
        fieldId = 0,
        dataType = 0,
        data = repeatedItem
