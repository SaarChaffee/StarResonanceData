...
local mergeDataFuncs = {
  [1] = function(container, buffer, watcherList)
    ...
    for i = 1, add do
      ...
      container.Watcher:MarkMapDirty("colorInfoMap", dk, nil)
    end
    for i = 1, remove do
      local dk = br.ReadInt32(buffer)
      local last = container.colorInfoMap.__data__[dk]
      container.colorInfoMap.__data__[dk] = nil
...
local getContainerElem = function(container)
  ...
  local ret = {}
  if container.colorInfoMap ~= nil then
    local data = {}
    for key, repeatedItem in pairs(container.colorInfoMap) do
      data[key] = {
        fieldId = 0,
        dataType = 0,
        data = repeatedItem
