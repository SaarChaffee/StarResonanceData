...
local mergeDataFuncs = {
  ...
  [2] = function(container, buffer, watcherList)
    ...
    for i = 1, add do
      ...
      container.colors.__data__[dk] = v
      container.Watcher:MarkMapDirty("colors", dk, nil)
    end
    for i = 1, remove do
      local dk = br.ReadInt32(buffer)
      local last = container.colors.__data__[dk]
      container.colors.__data__[dk] = nil
      container.Watcher:MarkMapDirty("colors", dk, last)
    end
    for i = 1, update do
      local dk = br.ReadInt32(buffer)
      local last = container.colors.__data__[dk]
      if last == nil then
        ...
        container.colors.__data__[dk] = last
      end
...
local resetData = function(container, pbData)
  ...
  for k, v in pairs(pbData.colors) do
    container.colors.__data__[k] = require("zcontainer.int_vec3").New()
...
local getContainerElem = function(container)
  ...
  if container.colors ~= nil then
    local data = {}
    for key, repeatedItem in pairs(container.colors) do
