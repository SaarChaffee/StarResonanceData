...
local checkValid = function(itemUuid, configId, data)
  ...
  if itemsVM.CheckPackageTypeByItemUuid(itemUuid, E.BackPackItemPackageType.Mod) and data and data.viewConfigKey and data.viewConfigKey == "mod_window" and not modVM.IsModEquip(itemUuid) then
...
local loadRedNode = function(itemUuid, configId)
  if itemUuid and configId then
    return E.RedType.ModTab .. itemUuid
  end
end
local ret = {
