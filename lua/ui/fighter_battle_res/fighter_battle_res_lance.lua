...
local fighterBattleResLance = class("fighterBattleResLance", super)

function fighterBattleResLance:ctor(rect, viewParent)
  super.ctor(self, rect, viewParent)
  self.resDot_ = dot.new()
  self.uibinder_ = nil
end

function fighterBattleResLance:OnActive()
  ...
  self.displayComp_ = {
    [E.PlayerBattleResType.BarEffect] = {
      [E.PlayerBattleResType.BarEffect] = self.uibinder_.main_lance.ui_display_eff_1,
      [E.PlayerBattleResType.Bar] = self.uibinder_.main_lance.ui_display_eff_2
    },
    [E.PlayerBattleResType.Bar] = self.uibinder_.main_lance.img_lance_bottom_on,
    [E.PlayerBattleResType.DotEffect] = self.resDot_
  }
  self.uibinder_.main_lance.img_lance_bottom_on:Play(0, 0, 0, nil)
  self.uibinder_.main_lance.Ref.UIComp.UIDepth:AddChildDepth(self.uibinder_.main_lance.ui_display_eff_1)
  self:OnBuffChange()
end

function fighterBattleResLance:OnRefresh()
  for index, value in ipairs(self.fightResTemplateRow_.ResIDs) do
    if self.ResValue[value] then
      ...
      elseif self.fightResTemplateRow_.UIType[index] == E.ResUIType.Line and self.ResIdOpen[value] then
        self.uibinder_.main_lance.img_lance_bar:SetFilled(nowNum / maxNum, true)
        self.uibinder_.main_lance.lab_progress.text = nowNum .. "/" .. maxNum
      end
    end
  end
end

function fighterBattleResLance:OnBuffChange()
...
function fighterBattleResLance:refreshShowRes()
  self.uibinder_.dot.Ref.UIComp:SetVisible(self.TypeOpen[E.ResUIType.Dot])
  self.uibinder_.main_lance.Ref:SetVisible(self.uibinder_.main_lance.line_root, self.TypeOpen[E.ResUIType.Line])
end

function fighterBattleResLance:refreshDisplayEffect()
  for index, value in ipairs(self.fightResTemplateRow_.EffectShowBuff) do
    ...
    if displayFunc then
      if type == E.PlayerBattleResType.Bar then
        ...
        displayEffectParam.effPath = "ui/uieffect/prefab/ui_sfx_battle_weapon_001/ui_sfx_group_cq_fankui_once"
...
function fighterBattleResLance:OnBattleResCdChange(resId, fightResCd)
  self.resDot_:OnBattleResCdChange(resId, fightResCd)
end

function fighterBattleResLance:OnDeActive()
  self.resDot_:DeActive()
end

return fighterBattleResLance
