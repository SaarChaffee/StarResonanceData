...
local fighterBattleResSnow = class("fighterBattleResSnow", super)

function fighterBattleResSnow:ctor(rect, viewParent)
  super.ctor(self, rect, viewParent)
  self.resDot_ = dot.new()
  self.uibinder_ = nil
end

function fighterBattleResSnow:OnActive()
  ...
  self.displayComp_ = {
    [E.PlayerBattleResType.BarEffect] = {
      [E.PlayerBattleResType.BarEffect] = self.uibinder_.main_snow.ui_display_eff_1,
      [E.PlayerBattleResType.Bar] = self.uibinder_.main_snow.ui_display_eff_2
    },
    [E.PlayerBattleResType.Bar] = self.uibinder_.main_snow.img_snow_bottom_on,
    [E.PlayerBattleResType.DotEffect] = self.resDot_
  }
  self.uibinder_.main_snow.img_snow_bottom_on:Play(0, 0, 0, nil)
  self.uibinder_.main_snow.Ref:SetVisible(self.uibinder_.main_snow.img_snow_on, false)
  self:OnBuffChange()
end

function fighterBattleResSnow:OnRefresh()
  for index, value in ipairs(self.fightResTemplateRow_.ResIDs) do
    if self.ResValue[value] then
      ...
      elseif self.fightResTemplateRow_.UIType[index] == E.ResUIType.Line and self.ResIdOpen[value] then
        self.uibinder_.main_snow.img_snow_bar:SetFilled(nowNum / maxNum, true)
        self.uibinder_.main_snow.lab_progress.text = nowNum .. "/" .. maxNum
      end
    end
  end
end

function fighterBattleResSnow:OnBuffChange()
...
function fighterBattleResSnow:refreshDisplayEffect()
  for index, value in ipairs(self.fightResTemplateRow_.EffectShowBuff) do
    ...
    if displayFunc then
      if type == E.PlayerBattleResType.Bar then
        ...
        displayEffectParam.effPath = "ui/uieffect/prefab/ui_sfx_battle_weapon_001/ui_sfx_group_fz_fankui_once"
    ...
    if type == E.PlayerBattleResType.BarEffect and self.lastEffectOpen_[type] ~= displayEffectParam.isOpen then
      self.uibinder_.main_snow.Ref:SetVisible(self.uibinder_.main_snow.img_snow_on, displayEffectParam.isOpen)
    end
    self.lastEffectOpen_[type] = displayEffectParam.isOpen or false
  end
end

function fighterBattleResSnow:refreshShowRes()
  self.uibinder_.dot.Ref.UIComp:SetVisible(self.TypeOpen[E.ResUIType.Dot])
  self.uibinder_.main_snow.Ref:SetVisible(self.uibinder_.main_snow.line_root, self.TypeOpen[E.ResUIType.Line])
end

function fighterBattleResSnow:OnBattleResCdChange(resId, fightResCd)
  self.resDot_:OnBattleResCdChange(resId, fightResCd)
end

function fighterBattleResSnow:OnDeActive()
  self.resDot_:DeActive()
end

return fighterBattleResSnow
