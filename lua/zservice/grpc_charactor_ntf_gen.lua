...
local impl = require("zservice/grpc_charactor_ntf_impl")
...
local OnCallStub = function(call)
  xpcall(function()
    if call:GetMethodId() == 1 then
      ...
      local pbMsg = pb.decode("zproto.GrpcCharactorNtf.GetFaceUpTokenNtf", pbData)
      if MessageInspectBridge.InInspectState == true then
        MessageInspectBridge.HandleReceiveMessage(1953836151, 1, cJson.encode(pbMsg), pbData, true)
      end
      impl:GetFaceUpTokenNtf(call, pbMsg.vRequest)
      return
    end
  end, function(err)
    logError([[
...
stub:Init(1953836151, "GrpcCharactorNtf", OnCreateStub, OnCallStub)
